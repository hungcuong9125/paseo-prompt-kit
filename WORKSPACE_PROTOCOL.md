...
- Desktop/Web QA: manual matrix in `docs/IMPELEMENT_PLAN.md` §22 Phase 7, run by the QA/verify seat against a local Paseo daemon with the plugin installed from this checkout; browser evidence via the Human-provided MultiZen endpoint when the seat has it configured, else `UNKNOWN` with the reason.
