...
| DEF-008 | The daemon rejects any plugin RPC after `REQUEST_TIMEOUT_MS = 30_000` (`@getpaseo/server` `plugins/runtime.ts`, verified in the installed Paseo 0.8.0), while `prompt-kit` advertises `timeoutMs` up to 600_000. A rewrite that outlives 30s therefore fails in the host with `Plugin RPC timed out` before the plugin's own timeout can fire, and a user-chosen 90s budget is unreachable. Measured cold runs: `opencode` 36.3s, `codex` 41.4s — both above the cap. The CLI runner still kills the process tree at its own timeout, so no process is leaked; only the reported failure is the wrong one | 2026-09-21 | DLF-012 / live two-path probe | The plugin's `timeoutMs` is lowered under the cap, the rewrite is made resumable, or upstream raises `REQUEST_TIMEOUT_MS` for `plugin.rpc.invoke` | OPEN |
